...
// Precomputed once (Demo D4): the scan depends only on immutable seed fields, so it
// no longer blocks the event loop on every request.
let relatedCache: Map<string, number> | null = null;
...
export async function ticketRoutes(app: FastifyInstance) {
  ...
  app.get("/api/tickets", async () => {
    ...
    const related = relatedCache ?? (relatedCache = relatedCounts(tickets));

...
export function App() {
  ...
  // Load the queue with the three states the UI actually needs (Demo D2): a loading
  // signal while the request is in flight, and a distinct error branch (with retry via
  // reloadKey) instead of a silently blank queue when the fetch fails.
  useEffect(() => {
    let active = true;
    setStatus("loading");
    fetchTickets()
      ...
      <main className="app__main">
        <section className="hero-panel">
          ...
          <div className="hero-panel__visual">
            {/* LCP image (Demo D4): right-sized WebP variants, space reserved via width/height,
                and prioritized (fetchpriority + preload in index.html). */}
            <picture>
              <source
                type="image/webp"
        ...
        <div className="workspace">
          <section id="queue" className="panel panel--queue">
            ...
            {status === "error" && (
              <div className="queue-note queue-note--error" role="alert">
                <span>Couldn’t load the queue.</span>
                <button
                  type="button"
                  className="btn"
          ...
          <aside className="side-stack">
            ...
            <Suspense fallback={<section className="panel panel--insights" aria-busy="true"><p className="queue-note">Loading reporting…</p></section>}>
